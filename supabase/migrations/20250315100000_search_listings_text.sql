-- Wyszukiwarka tekstowa: tytuł, opis, tagi AI ze zdjęć
-- RPC zwraca listę id ogłoszeń spełniających frazę + filtry (RLS stosowane).
...
DECLARE
  search_escaped TEXT;
BEGIN
  -- Escape % i _ dla bezpiecznego ILIKE (użytkownik nie może rozszerzyć wzorca)
    ...
    AND (
      ...
      OR EXISTS (
        ...
        FROM public.listing_photos lp,
             unnest(COALESCE(lp.ai_tags, ARRAY[]::text[])) AS t(tag)
        WHERE lp.listing_id = l.id
          AND t.tag ILIKE '%' || search_escaped || '%' ESCAPE E'\\'
      )
    )
...
COMMENT ON FUNCTION public.search_listings IS 'Zwraca id ogłoszeń aktywnych pasujących do frazy (tytuł, opis, tagi AI) i opcjonalnych filtrów.';
